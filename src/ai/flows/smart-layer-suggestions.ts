...
const SmartLayerSuggestionsOutputSchema = z.array(z.object({
  ...
  rationale: z.string().describe("A concise, user-friendly explanation of why this layer is a good choice for the given data schema."),
  initialConfiguration: z.object({}).catchall(z.any()).describe("A valid deck.gl layer configuration object for the suggested layerType."),
}));
export type SmartLayerSuggestionsOutput = z.infer<typeof SmartLayerSuggestionsOutputSchema>;

// Default ScatterplotLayer suggestion
const DEFAULT_SCATTERPLOT_LAYER: SmartLayerSuggestionsOutput[number] = {
  layerType: "ScatterplotLayer",
  rationale: "Ideal for visualizing individual locations. Each point on the map represents a row in your data.",
  initialConfiguration: {
    opacity: 0.8,
    ...
    radiusMinPixels: 3,
    radiusMaxPixels: 100,
    getFillColor: [255, 140, 0, 180] as [number, number, number, number],
  },
};
...
Provide up to 3 relevant layer suggestions from: ["ScatterplotLayer", "HeatmapLayer", "HexagonLayer", "ScreenGridLayer", "ColumnLayer"].

Rules:
1. Always include ScatterplotLayer as the first option.
2. Suggest density layers (HeatmapLayer, HexagonLayer, ScreenGridLayer) if dataset represents point data.
3. Suggest ColumnLayer if a numeric value is mapped and represents magnitude.
4. Provide rationale and initialConfiguration (no 'id' or 'data'). Use RGBA arrays for colors.
...
const suggestLayersFlow = ai.defineFlow(
  ...
  async (input: SmartLayerSuggestionsInput): Promise<SmartLayerSuggestionsOutput> => {
    try {
      const { output } = await smartLayersPrompt(input);

      // If AI returns nothing, fallback to default
      if (!output || output.length === 0) {
        return [DEFAULT_SCATTERPLOT_LAYER];
      }

      // Ensure ScatterplotLayer is first
      const scatterIndex = output.findIndex(l => l.layerType === 'ScatterplotLayer');
      if (scatterIndex > 0) {
        const scatter = output.splice(scatterIndex, 1)[0];
        output.unshift(scatter);
      } else if (scatterIndex === -1) {
        output.unshift(DEFAULT_SCATTERPLOT_LAYER);
      }

      return output;
    } catch (error) {
      console.error("Error in suggestLayersFlow:", error);
      return [DEFAULT_SCATTERPLOT_LAYER];
    }
  }
);
